...
def getPelicanaAddress(result):
    ...
    for page_idx in count():
        ...
        Pelicana_URL = 'http://www.pelicana.co.kr/store/stroe_search.html?page=%s&branch_name=&gu=&si=' %str(page_idx + 1) #내가
        print('[Pericana Page] : [%s]'%(str(page_idx+1)))

        rcv_data= get_request_url(Pelicana_URL)
        soupData = BeautifulSoup(rcv_data,'html.parser')
        
        store_table = soupData.find('table', attrs={'class':'table mt20'}) # 내가
...
def main():
    ...
    print('PERLCANA ADDRESS CRAWLING START')
